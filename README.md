...
| 入札カーブ | 受渡日・時間帯ごとの売り・買いの入札カーブと交点。対象にエリアを選ぶと、日付や時間帯を変えても、そのエリアの価格を決めたカーブ（市場分断していれば分断エリアのカーブ、1 エリアだけで分断した単エリアは推定）を表示。カーブの比較（直近 7 日・6 時間おき・自由に選んだ日と時間帯）。表示中のカーブの入札の大きな段（その価格で増えた量の大きい順）。単エリアが 1 つだけのコマ（そのエリアのカーブを推定し、約定価格で補正できるコマ）を、エリア別と時間帯別のコマ数、日付 × 時間帯の図で探せ、押すとそのコマのカーブを表示（前後のそのようなコマへ移るボタンと、直近のそのようなコマを比較の図に重ねるボタンもある）。そのようなコマの推定カーブから各コマの大きい 10 段を集め、同じような価格と量の段が何コマに出てくるか（何度も出てくる段）を、価格 × 量の図と上位 10 の一覧で表示。価格感応度として、1 コマの買いの増減と約定価格の図（約定価格が 0.01 円になる量、20 円と 50 円を超える量、売りが尽きる量、買いを ±0.5GW、±1GW、±5GW 増減したときの価格と JEPX の公表値）、エリア別の価格感応度と 0.01 円や高騰の目安までの買いの増減（押すと対象をそのエリアにする）と、それらの推移（システムプライスは JEPX の公表値と入札カーブから計算した目安、エリアはボタンを押すとカーブを 1 日ずつ読んで計算）。1 コマの図とエリア別の図には、ブロック入札の約定が変わって効かない分を公表値から見込んだ値と、その範囲も重ねる。期間で見る図として、価格帯ごとの入札量（0.01 円以下の売り、最も高い価格の買いなど）の推移、指標のヒートマップ。`npm run fetch` で取得した入札カーブを使います（デモでは合成したカーブを表示） |
...
- 取引結果に JEPX の価格感応度の公表値があれば、システムプライスのカーブをずらして計算した目安と比べます（買いの増減ごとに、約定価格の動きが公表値と同じコマの割合、差の大きさの平均、公表値の動きが目安以下のコマの割合、足した量のうち効かなかった割合）
...
ブロック入札の約定が変わる分の見込み（集計の定義を参照）も、2026 年 8 月 28 日〜9 月 26 日の実データで試しました。
各日の前の 7 日の公表値から効かなかった割合を求め、その日の公表値を当てると、差の大きさの平均は次のとおりでした（23 日、1 つの量と向きあたり 1,104 コマ）。

| 買いの増減 | カーブをずらしただけ | 見込み | 25〜75% の範囲に入った公表値 |
| --- | --- | --- | --- |
| +0.5GW / −0.5GW | 0.20 / 0.19 円 | 0.19 / 0.19 円 | 67% / 68% |
| +1GW / −1GW | 0.36 / 0.34 円 | 0.32 / 0.27 円 | 71% / 73% |
| +5GW / −5GW | 1.82 / 1.60 円 | 0.91 / 0.75 円 | 56% / 55% |

そのコマの ±0.5GW と ±5GW の公表値から割合を決め、その間を直線で結んで ±1GW を当てると、差の大きさの平均は 0.39 円から 0.23 円になりました。
効かなかった量の多さは、取引結果のブロック入札の量（約定しなかった売りのブロック入札など）とはほとんど関係がありませんでした（相関係数 0.33 以下）。
...
| ブロック入札の変化の見込み | JEPX の公表値の計算では、ブロック入札の約定も判定し直すため、足した量の一部が効かない（買いを足して価格が上がると、それまで約定しなかった売りのブロック入札が約定するなど）。公表値の価格になるようにカーブをずらす量と足した量との差を「効かなかった量」とし、足した量に対する割合で見込む（公表値の価格になるずらし方が幅を持つときは、足した量に最も近いもの）。割合は ±0.5GW、±1GW、±5GW の量ごとに求め、その間と 0 との間は、効かない量を量について直線で結ぶ。システムプライスのカーブ（分断していないエリアも同じカーブ）は、そのコマの公表値に合うように割合を決める（見込みは公表値を通る）。分断エリアや単エリアのカーブは、表示している日までの直近 7 日のシステムプライスの公表値から求めた割合の中央値で見込み、範囲は 25〜75% 点とする。5GW を超える分は、効かない量が 5GW のときから増えない場合と、直近 7 日の 5GW での割合（75% 点、上限 75%）で増える場合の間を範囲とし、見込みは中央値の半分の割合で増やす。1 コマの図とエリア別の図だけに重ね、期間の図には重ねない |
