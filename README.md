...
| 推移 | 30 分値〜日・週・月・年度・暦年の粒度で、平均・最高・最低・中央値の推移。1 系列なら最低〜最高の帯を表示。系列ごとに分割した小さな図（縦軸共通）でも比較可 |
...
| エリア比較 | システムプライスとの平均差、市場分断の発生率、2 エリア間の値差の推移（例: 東京−関西）、エリア間で価格が異なったコマの割合（9×9）、月別の値差 |
...
| 市場分断の発生率 | エリアプライスがシステムプライスと異なった（差が 0.005 円超の）コマの割合 |
...
| 時間帯 | 時刻コード 1〜48 を 0:00〜23:30 開始の 30 分コマとして扱う。終了時刻が開始時刻より前の指定（例: 20:00〜8:00）は日をまたぐ |
