...
| 入札カーブ | 受渡日・時間帯ごとの売り・買いの入札カーブと交点。対象にエリアを選ぶと、日付や時間帯を変えても、そのエリアの価格を決めたカーブ（市場分断していれば分断エリアのカーブ、1 エリアだけで分断した単エリアは推定）を表示。カーブの比較（直近 7 日・6 時間おき・自由に選んだ日と時間帯）。期間で見る図として、価格帯ごとの入札量（0.01 円以下の売り、最も高い価格の買いなど）と価格感応度（買いが 1GW 増減したときの価格の変化）の推移、指標のヒートマップ。`npm run fetch` で取得した入札カーブを使います（デモでは合成したカーブを表示） |
...
- システムプライスのカーブの入札量の合計が、取引結果の売り・買い入札量（全国）と同じか（市場分断・単エリアの有無ごと、月ごと）。同じなら、システムプライスのカーブには単エリアを含む全エリアの入札が入っています
- 公表されている分断エリアのカーブが、そのエリアの約定価格で交わるか。交わるなら、連系線でやりとりする量も入っています
- 公表されている分断エリアのカーブの合計 − システムプライスのカーブ（単エリアが無いコマでは、連系線でやりとりする量）
- 単エリアが 1 つのコマで、推定したカーブの交点がそのエリアの約定価格と合うか。1 コマを詳しく見るときは、価格ごとの「システムプライス − 分断エリアの合計」も出します（価格によって変われば単エリアの入札が入っていて、価格によらず一定の分が連系線でやりとりする量）
...
| 分断エリアのカーブ | 市場分断したコマで JEPX が公表する、2 エリア以上のまとまりのカーブ。連系線で他の分断エリアとやりとりする量が、送る側では買い、受ける側では売りとして（価格によらない量で）入っているとみられる（そのため、そのエリアの約定価格で交わる。公表されている分断エリアの入札量の合計が、システムプライスより多くなることもある）。システムプライスのカーブは全エリアの入札そのものとみられる（それなら入札量の合計は取引結果の売り・買い入札量と同じになる）。どちらも `npm run check:curves` で確かめられる |
| 単エリアのカーブ（推定） | 1 エリアだけで分断したエリア（単エリア）は JEPX がカーブを公表していないため、システムプライスのカーブから、公表されている分断エリアのカーブを引いて推定する（破線で表示）。引くと、分断エリアのカーブに入っている連系線の量の分だけ売り・買いとも引き過ぎになるので、売り・買いに同じ量を足して 0 以上に戻す（同じ量なので交点の価格は変わらない。足す量そのもの（単エリアの最も安い売り・最も高い買いの量）は分からないため、少ない方の端を 0 にする。横軸の位置は目安）。単エリアが 1 つのときは、そのエリアの約定価格で交わるはずなので、ずれが残れば売りか買いの一方に足して合わせる（約定価格での売りの量が買いより多ければ買いに、少なければ売りにその差を足す）。単エリアが複数のときはエリアごとに分けられないため、合わせたカーブ（補正なし）と各エリアの約定価格を表示し、同じ日のうち、そのエリアだけが単エリアになった時間帯へ移るボタンを出す。引いた差が価格によらずほぼ一定で、単エリアの入札の形が残らないときは推定できないとして、システムプライスと分断エリアの入札量の合計を表示する |
...
```sh
npm test            # 単体テスト（CSV 解析・祝日判定・集計・入札カーブ・取得スクリプト・1 ファイル版の作成・入札カーブの確認）
