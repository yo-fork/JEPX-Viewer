...
ヒートマップとカレンダーでは色の範囲を、時間帯タブと分布タブの箱ひげ図では縦軸の範囲を、既定では対象（システムプライスまたはエリア）ごとに決めます。
そのため対象を切り替えると範囲も変わり、色や箱の位置をそのままでは比べられません。
...
- 1 コマを詳しく見るときは、入札量の合計、取引結果（入札量・ブロック入札の量・約定総量）、ブロック入札の約定の違い、価格ごとの「システムプライス − 分断エリアの合計」、約定総量との突き合わせも出します
...
| 全エリア共通の範囲 | ヒートマップとカレンダーの色は、システムプライスと 9 エリアのそれぞれで「対象ごと」の範囲を決め、それらをすべて含む範囲（システムプライスとの差は 9 エリアで決める）。箱ひげ図の縦軸は、システムプライスと 9 エリアのひげ（10〜90%点）がすべて入る範囲。期間などの絞り込みや、ヒートマップの格子、カレンダーの指標、箱ひげ図のグループを変えると決め直す |
