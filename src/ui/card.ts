...
export class ChartCard {
  ...
  /** このカードだけに効く表示設定を、見出しの下に 1 行ずつ並べる（その行の要素を返す） */
  addControls(...items: HTMLElement[]): HTMLElement {
    const row = h('div', { class: 'card-controls' }, items);
    this.el.insertBefore(row, this.chartEl);
    return row;
  }
