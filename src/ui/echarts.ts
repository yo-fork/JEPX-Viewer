...
function buildTheme(name: ThemeName): object {
  ...
  return {
    ...
    dataZoom: {
      borderColor: t.border,
      backgroundColor: 'transparent',
      fillerColor: name === 'light' ? 'rgba(42,120,214,0.10)' : 'rgba(57,135,229,0.24)',
      handleStyle: { color: t.raised, borderColor: name === 'light' ? t.axis : t.muted },
      moveHandleStyle: { color: name === 'light' ? t.axis : t.muted },
      textStyle: { color: t.muted },
      dataBackground: { lineStyle: { color: t.axis }, areaStyle: { color: t.grid } },
      selectedDataBackground: { lineStyle: { color: t.cat[0] }, areaStyle: { color: t.cat[0], opacity: t.bandAlpha } },
